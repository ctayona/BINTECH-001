...
function initializeGoogleSignIn() {
  ...
  console.log('[Google Auth] Google Sign-In initialization started');
  console.log('[Google Auth] Login button found:', !!loginButton);
  console.log('[Google Auth] Signup button found:', !!signupButton);
  console.log('[Google Auth] Client ID:', GOOGLE_CLIENT_ID ? GOOGLE_CLIENT_ID.substring(0, 20) + '...' : 'NOT SET');
  ...
  if (typeof window.google === 'undefined' || !window.google.accounts) {
    ...
    if (googleInitRetries <= 2 || googleInitRetries % 10 === 0) {
      console.warn('[Google Auth] Google Identity Services not yet loaded, retrying in 500ms... (attempt ' + googleInitRetries + ')');
    }
  ...
  if (loginButton) {
    try {
      ...
      console.log('[Google Auth] ✓ Login button rendered successfully');
    } catch (error) {
      console.error('[Google Auth] Error rendering login button:', error.message);
    }
  }
  ...
  if (signupButton) {
    try {
      ...
      console.log('[Google Auth] ✓ Signup button rendered successfully');
    } catch (error) {
      console.error('[Google Auth] Error rendering signup button:', error.message);
    }
  }
