...
exports.register = async (req, res) => {
  try {
    ...
    console.log(`Staff Account ID (for staff/external faculty): ${staffAccountId || 'None'}`);
    console.log(`ID Column Name: ${idColumnName}`);
    console.log(`Role Table: ${roleTableName}`);
    console.log(`=============================\n`);
    
    // Validate campus_id was extracted for UMAK students only
    // Faculty and staff can use external emails (will get generated account_id)
    ...
    if (userRole === 'student') {
      // Students: use student_id from email (K-number or A-number)
...
exports.googleLogin = async (req, res) => {
  try {
    ...
    try {
      ...
      if (userRole === 'faculty' && !campusId) {
        campusId = await generateStaffAccountId();
        console.log(`✓ Generated account_id for external faculty email: ${campusId}`);
      }
      ...
      if (userRole === 'student') {
        ...
        // Faculty: use faculty_id from UMak email OR account_id for external emails
        const extractedCampusId = extractCampusId(trimmedEmail, userRole);
        if (extractedCampusId) {
          // UMak faculty email (e.g., firstname.lastname@umak.edu.ph)
          roleSpecificData[idColumnName] = campusId;
          console.log(`Adding ${idColumnName}: ${campusId}`);
        } else {
          // External faculty email (e.g., Gmail) - use generated account_id
          roleSpecificData.account_id = campusId; // campusId is actually account_id for external faculty
          console.log(`Adding account_id for external faculty: ${campusId}`);
        }
      } else if (userRole === 'staff') {
