...
exports.getDashboardSummary = async (req, res) => {
  try {
    ...
    // 3B. System statistics - UPDATED with correct data sources
    const [
      userAccountsStats,
      adminAccountsStats,
    ...
    // Calculate total rewards redeemed (sum of quantities)
    const totalRewardsRedeemed = (redemptionsStats.data || []).reduce((sum, r) => sum + (Number(r.quantity) || 1), 0);

    // Calculate total waste sorted from machine_sessions (sum of all material counts)
    const { data: machineSessionsData, error: machineSessionsError } = await supabase
      .from('machine_sessions')
      .select('metal_count, plastic_count, paper_count');

    if (machineSessionsError) {
      console.warn('Error fetching machine sessions for waste totals:', machineSessionsError.message);
    }
    ...
    res.json({
      ...
      summary: {
        ...
        // Use total waste sorted from machine_sessions (sum of metal + plastic + paper counts)
        totalWasteSorted: totalWasteSortedFromSessions,
...
exports.getWebsiteLogs = async (req, res) => {
  try {
    ...
    const redemptionLogs = (redemptions || []).map((redemption) => {
      ...
      // Prefer the most specific account mapping first, then fall back to generic records.
      let account = (redemptionEmailKey ? userAccountMap.get(redemptionEmailKey) : null)
        || userAccountMap.get(redemptionUserKey)
        || userAccountMap.get(redemptionUserKey.toLowerCase())
        || (redemptionEmailKey ? userAccountsByUUID.get(redemptionEmailKey) : null)
      ...
      return {
        ...
        userName: account?.name || redemption.user_id || 'Unknown user',
...
// ============================================
// Analytics API - Get analytics data with date range filtering
// ============================================
exports.getAnalytics = async (req, res) => {
  try {
    ...
    // 1. Total Waste Sorted (SUM of metal_count + plastic_count + paper_count from machine_sessions)
    const { data: machineSessionsData, error: machineSessionsError } = await supabase
      .from('machine_sessions')
      .select('metal_count, plastic_count, paper_count, started_at')
      .gte('started_at', dateFrom + 'T00:00:00Z')
      .lte('started_at', dateTo + 'T23:59:59Z');

    if (machineSessionsError) {
      console.warn('Error fetching machine sessions:', machineSessionsError.message);
    }

    const totalWasteSorted = (machineSessionsData || []).reduce((sum, session) => {
      return sum + (Number(session.metal_count || 0) + Number(session.plastic_count || 0) + Number(session.paper_count || 0));
    }, 0);

    // 2. Total Points Generated (SUM of total_points from machine_sessions)
    const totalPointsGenerated = (machineSessionsData || []).reduce((sum, session) => {
      return sum + (Number(session.total_points || 0));
    }, 0);

    // 3. Active Users (COUNT of users with status = 'active')
    const { data: activeUsersData, error: activeUsersError, count: activeUsersCount } = await supabase
      .from('user_accounts')
      .select('system_id', { count: 'exact', head: true })
      .eq('status', 'active');

    if (activeUsersError) {
      console.warn('Error fetching active users:', activeUsersError.message);
    }

    const activeUsers = activeUsersCount || 0;

    // 4. Rewards Redeemed (SUM of quantity from completed redemptions)
    const { data: redemptionsData, error: redemptionsError } = await supabase
      .from('redemptions')
      .select('quantity, completed_at')
      .eq('status', 'completed')
      .gte('completed_at', dateFrom + 'T00:00:00Z')
      .lte('completed_at', dateTo + 'T23:59:59Z');

    if (redemptionsError) {
      console.warn('Error fetching redemptions:', redemptionsError.message);
    }

    const rewardsRedeemed = (redemptionsData || []).reduce((sum, r) => sum + (Number(r.quantity) || 1), 0);

    console.log('✓ Analytics data calculated:');
    console.log(`  Total Waste Sorted: ${totalWasteSorted}`);
    console.log(`  Total Points Generated: ${totalPointsGenerated}`);
    console.log(`  Active Users: ${activeUsers}`);
    console.log(`  Rewards Redeemed: ${rewardsRedeemed}`);
    console.log('===================================\n');

    res.json({
      success: true,
      analytics: {
        totalWasteSorted: totalWasteSorted,
...
// ============================================
// Analytics API - Get machine sessions data for charts
// ============================================
exports.getAnalyticsSessions = async (req, res) => {
  try {
    const dateFrom = req.query.dateFrom || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const dateTo = req.query.dateTo || new Date().toISOString().split('T')[0];
    ...
    const { data: sessions, error } = await supabase
      .from('machine_sessions')
      .select('id, metal_count, plastic_count, paper_count, total_points, started_at, status')
      .gte('started_at', dateFrom + 'T00:00:00Z')
      .lte('started_at', dateTo + 'T23:59:59Z')
      .order('started_at', { ascending: false });
...
// ============================================
// Analytics API - Get user data for charts
// ============================================
...
// ============================================
// Analytics API - Get redemptions data for charts
// ============================================
exports.getAnalyticsRedemptions = async (req, res) => {
  try {
    const dateFrom = req.query.dateFrom || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const dateTo = req.query.dateTo || new Date().toISOString().split('T')[0];
    ...
    const { data: redemptions, error } = await supabase
      .from('redemptions')
      .select('id, quantity, status, completed_at, requested_at')
      .gte('requested_at', dateFrom + 'T00:00:00Z')
    ...
    console.error('Error fetching analytics redemptions:', error);
    res.json({ success: true, data: [] });
  }
};
