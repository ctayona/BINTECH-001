...
async function getUserRankInfo(userId, email) {
  try {
    ...
    if (allUsersError || !allUsers || allUsers.length === 0) {
      console.log('[Rank] Error fetching leaderboard or no users found');
      return {
        notRanked: true,
        message: 'Not ranked yet'
      };
    ...
    let rank = 1;
    for (let i = 0; i < allUsers.length; i++) {
      if (allUsers[i].system_id === userPoints.system_id || 
          String(allUsers[i].email || '').toLowerCase() === String(userPoints.email || '').toLowerCase()) {
        rank = i + 1;
        break;
      }
    }
  ...
  try {
    ...
    return res.json({
      ...
      stats: {
        ...
        notRanked: rankInfo ? rankInfo.notRanked : true
      },
      logs: {
